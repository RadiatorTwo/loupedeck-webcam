namespace Loupedeck.WebcamPlugin.Actions
{
    ...
    public class PanAdjustment : ActionEditorAdjustment
    {
        ...
        public PanAdjustment()
            : base(hasReset: false)
        {
            this.DisplayName = "Pan";
            this.Description = "Set the Pan for the selected Webcam";
        ...
        private void OnActionEditorControlValueChanged(Object sender, ActionEditorControlValueChangedEventArgs e)
        {
            ...
            {
                e.ActionEditorState.SetDisplayName($"Set Pan for Webcam {e.ActionEditorState.GetControlValue(ListboxControlName)}");
            }
        }
            ...
            if (this._videoSource == null)
            {
                ...
                foreach (FilterInfo device in videoDevices)
                {
                    if (device.Name.Equals(devicename))
                    {
                        ...
                        this._videoSource.GetCameraPropertyRange(CameraControlProperty.Pan, out this.minValue, out this.maxValue, out this.defaultValue, out this.stepSize, out cameraControlFlags);
                        this._videoSource.GetCameraProperty(CameraControlProperty.Pan, out this.currentValue, out cameraControlFlags);
        ...
        private void SetValue() => this._videoSource.SetCameraProperty(CameraControlProperty.Pan, this.currentValue, CameraControlFlags.Manual);
